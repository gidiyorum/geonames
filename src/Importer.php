...
class Importer
{
     ...
     *
     * @param \Gidiyorum\Geonames\RepositoryInterface $repository
     */
    public function __construct(RepositoryInterface $repository)
    {
        $this->repository = $repository;
        $this->featureClasses = ['A', 'P'];
        $this->featureCodes = [
            'AIRP', 'CONT',
            // 'ADM1', 'ADM2', 'PPLA', 'PPLA2', 'PCLI',
            // 'PPL', 'ADM3', 'PPLA3',
        ];
    }
    ...
    public function names($table, $path)
    {
        ...
        $this->parseFile($path, function ($row) use ($table, $repository) {
            ...
            if (in_array($insert['f_class'], $this->featureClasses) || in_array($insert['f_code'], $this->featureCodes)) {
    ...
    public function alternateNames($table, $path)
    {
        ...
        $this->parseFile($path, function ($row) use ($table, $repository) {
            ...
            if ('' != $insert['iso_language'] and !$row[5]) {
                $repository->insert($table, $insert);
            }
        });
    }
    ...
    protected function parseFile($path, $callback)
    {
        ...
        while (!feof($handle)) {
            ...
            if (!$line or '' === $line or 0 === strpos($line, '#')) {
                continue;
            }
